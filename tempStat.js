...
const gl_versionNr = "v1.39"
const gl_versionDate = "10.8.2025"
    ...
    paint() {
        ...
        if (this.backColor != "") {
            for (place = 1; place <= nrPlaces; place++) {
                ...
                if (!placeSubject[place][gl_subject]) { color = this.disabledColor } // 11.8.2024
                switch (place) {
...
class intChooser2H {
    constructor(left, top, items, value, minItemValue, step, focusMargin, arrowWidth, arrowHeight, arrowGap, arrowColor, color, fillColor, crossColor, text, font, gap, posAlign, textColor, clickGap, enabled, disabledColor, disabledTextColor, visible, toolTipText, keyStroke) {
        ...
        this.enabled = enabled; this.disabledColor = disabledColor; this.disabledTextColor = disabledTextColor;
        this.visible = visible;
        this.toolTipText = toolTipText; this.keyStroke = keyStroke;
        //---- additional object properties
        this.width = 100;
        this.height = 30;
    }
...
//---- 11.8.2024 kaj prikazujemo - temperatura, nivo gladine vode, ...
const cv_subject_temp = 1;       //temperature
const cv_subject_waterLevel = 2; //water level
var gl_subject = cv_subject_temp;
//var gl_subject = cv_subject_waterLevel;
var placeSubject = [];    // kaj od podatov podpira posamezna postaja
...
//========================= ŠE ZA VIŠINO VODE ================================= (23.7.2024)

//---- nedefinirani intervali podatkov določenih lokacij (23.7.2024)
var nrUndefDataPeriods = 0;
const undefDataPeriodHeightPlace = [];
const undefDataPeriodHeightMonthStart = [];
const undefDataPeriodHeightYearStart = [];
const undefDataPeriodHeightMonthEnd = [];
const undefDataPeriodHeightYearEnd = [];
const undefDataPeriodHeightMonthIndexStart = [];      // od katerega podatka v globalnem razponu podatkov podatki niso definirani
const undefDataPeriodHeightMonthIndexEnd = [];        // do katerega podatka v globalnem razponu podatkov podatki niso definirani
const undefDataPeriodHeightPlaceMonthIndexStart = []; // od katerega podatka v nizu podatkov za to lokacijo podatki niso definirani
const undefDataPeriodHeightPlaceMonthIndexEnd = [];   // do katerega podatka v nizu podatkov za to lokacijo podatki niso definirani

//---- to se fila sproti med nalaganjem izmerjenih podatkov (23.7.2024)
const minMonthHeight = [];
const minYearHeight = [];
const maxMonthHeight = [];
const maxYearHeight = [];
const nrMonthsHeight = [];

//---- to se nafila naknadno, ko so vsi izmerjeni podatki že naloženi (23.7.2024)
const offsetMonthsHeight = [];  // 23.7.20224 za koliko mesecev je začetek podatkov lokacije zamaknjen glede na generalno prvi mesec podatkov
const firstMonthHeight = [];    // 23.7.20224   prvi mesec podatkov lokacije, zaporedno relativno glede na prvi mesec podatkov generalno
const lastMonthHeight = [];     // 23.7.20224 zadnji mesec podatkov lokacije, zaporedno relativno glede na prvi mesec podatkov generalno

var minMonthHeightAll = 20;  // mesec prvega podatka generalno od vseh lokacij (1..12)
var maxMonthHeightAll = 0;   // mesec zadnjega podatka generalno od vseh lokacij (1..12)
var minYearHeightAll = 2030; // leto prvega podatka generalno od vseh lokacij (recimo 2009)
var maxYearHeightAll = 0;    // leto zadnjega podatka generalno od vseh lokacij (recimo 2023)
var nrMonthsHeightAll = 0;   // število vseh mesečnih podatkov gledano generalno od vseh lokacij (če so podatki med avg'2009 in nov'2023, je to 172 =5+14*12-1)

var avgHeight = [];
var avgHeightCache = []; 
var avgHeightCacheAll = []; 
...
function addPlace(name, shortName, abbr, location, color, colorAnti) {
    ...
    //---- 23.7.2024
    minMonthHeight[nrPlaces] = 0;
    minYearHeight[nrPlaces] = 0;
    maxMonthHeight[nrPlaces] = 0;
    maxYearHeight[nrPlaces] = 0;
    nrMonthsHeight[nrPlaces] = 0;
    //---- 23.7.2024
    avgHeight[nrPlaces] = [];
    avgHeightCache[nrPlaces] = [];
    //----
    //---- 11.8.2024 podpora posameznih merjenih količin se vpisuje sproti pri filanju podatkov
    placeSubject[nrPlaces] = [];
    placeSubject[nrPlaces][cv_subject_temp] = false;
    placeSubject[nrPlaces][cv_subject_waterLevel] = false;
    //----
    return nrPlaces; //14.12.2023
}

function addUndefTempPlaceDataPeriod(place, monthStart, yearStart, monthEnd, yearEnd) {
    //---- 14.12.2023
...
function addAvgTempYear(vp_place, vp_year, vp_month, vp_arrTemp) {
    ...
    //---- ta postaja podpira podatke o temperaturi 11.8.2024
    placeSubject[vp_place][cv_subject_temp] = true; 

}

function addAvgWaterLevelYear(vp_place, vp_year, vp_month, vp_arrHeight) {
    //---- 23.7.2024 -----------------------------
    // vp_place         ... za katero lokacijo meritev
    // vp_year,vp_month ... od katerega meseca/leta naprej so ti podatki
    // vp_arrHeight     ... polje podatkov o višini vode za podani mesec in naprej proti koncu leta (ni nujno, da čisto do konca!!)
    //--------------------------------------------

    //let vl_month = (13 - minMonth[vp_place]) + 12 * (maxYear[vp_place] - minYear[vp_place] - 1) + maxMonth[vp_place];
    
    //---- za koliko mesecev so prisotni podatki v vp_arrHeight[]
    let nrData = vp_arrHeight.length;
    if (nrData <= 0) { return };

    //---- začetni mesec podatkov za to lokacijo
    if (minMonthHeight[vp_place] == 0) {
        minMonthHeight[vp_place] = vp_month;
        minYearHeight[vp_place] = vp_year;
    };
    if (monthYearBeforeMonthYear(vp_month, vp_year, minMonthHeightAll, minYearHeightAll)) {
        minMonthHeightAll = vp_month;
        minYearHeightAll = vp_year;
    }
      
    //---- končni mesec podatkov za to lokacijo
    let vl_monthEnd = vp_month + nrData - 1;
    maxMonthHeight[vp_place] = vl_monthEnd;
    maxYearHeight[vp_place] = vp_year;
    if (monthYearAfterMonthYear(vl_monthEnd, vp_year, maxMonthHeightAll, maxYearHeightAll)) {
        maxMonthHeightAll = vl_monthEnd;
        maxYearHeightAll = vp_year;
    }

    //---- vpis podatkov
    let vl_month, vl_monthIndex
    for (vl_monthIndex = 1; vl_monthIndex <= nrData; vl_monthIndex++) {
        vl_month = nrMonthsHeight[vp_place] + vl_monthIndex;
        avgHeight[vp_place][vl_month] = vp_arrHeight[vl_monthIndex - 1];
        avgHeightCache[vp_place][vl_month] = avgHeight[vp_place][vl_month]; // 19.12.2023
    }
        
    //---- število mesecev s podatki za to lokacijo
    nrMonthsHeight[vp_place] += nrData;
    nrMonthsHeightAll = 12 * (maxYearHeightAll - minYearHeightAll) + (maxMonthHeightAll - minMonthHeightAll) + 1;

    //---- ta postaja podpira ta podatek 11.8.2024
    placeSubject[vp_place][cv_subject_waterLevel] = true;
...
//addNormalizePeriod(cv_placeKredarica, station, 1, 1955, 12, 2023, 11.5);
...
addUndefTempPlaceDataPeriod(cv_placeBabnoPolje, 7, 1991, 10, 2003); //tu vmes ni podatkov, vseeno pa ohranim eno lokacijo, le graf se tu vmes ne bo prikazoval
...
addUndefTempPlaceDataPeriod(cv_placeVelenje, 4, 1999, 12, 2004); //tu vmes ni podatkov, vseeno pa ohranim eno lokacijo, le graf se tu vmes ne bo prikazoval
...
// LITIJA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 29.12.2023  
const cv_placeLitija = addPlace("Litija (266m)", "Litija", "LITIJA", "Litija", "limeGreen", "white");
station = addStation(cv_placeLitija, 2649, "LITIJA", "14.8185", "46.0651", "A", 266, 12, 2014, 0, 0);

// LISCA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeLisca = addPlace("Lisca (947m)", "Lisca", "LISCA", "Lisca", "deepPink", "white");
station = addStation(cv_placeLisca, 1846, "LISCA", "15.2849", "46.0678", "A", 947, 12, 1996, 0, 0);

// TRBOVLJE  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeTrbovlje = addPlace("Trbovlje (284m)", "Trbovlje", "TRBV", "Trbovlje", "coral", "white");
station = addStation(cv_placeTrbovlje, 1825, "TRBOVLJE", "15.0355", "46.1389", "A", 260, 3, 2008, 3, 2021);
station = addStation(cv_placeTrbovlje, 3395, "TRBOVLJE", "15.0539", "46.1575", "A", 284, 4, 2021, 0, 0);

// VELIKE LAŠČE  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeVelikeLasce = addPlace("Velike La" + scSchLow + scTchLow + "e (528m)", "Velike La" + scSchLow + scTchLow + "e", "VLA" + scSch + scTch + "E", "Velike La" + scSchLow + scTchLow + "e", "mediumTurquoise", "white");
station = addStation(cv_placeVelikeLasce, 2611, "VELIKE LA" + scSch + scTch + "E", "14.6427", "45.8310", "A", 528, 12, 2015, 0, 0);

// ILIRSKA BISTRICA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeIlirskaBistrica = addPlace("Ilirska Bistrica (415m)", "Ilirska Bistrica", "ILIRB", "Ilirska Bistrica", "darkOrchid", "white");
station = addStation(cv_placeIlirskaBistrica, 2250, "ILIRSKA BISTRICA - KOSEZE", "14.2355", "45.5531", "A", 415, 2, 2005, 0, 0);

// ROGAŠKA SLATINA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeRogaskaSlatina = addPlace("Roga" +scSchLow + "ka Slatina (250m)", "Roga" +scSchLow + "ka Slatina", "RSLAT", "Roga" +scSchLow + "ka Slatina", "darkSlateBlue", "white");
station = addStation(cv_placeRogaskaSlatina, 1867, "ROGA" +scSch + "KA SLATINA", "15.6437", "46.2391", "A", 250, 8, 1997, 12, 2009);
station = addStation(cv_placeRogaskaSlatina, 2837, "ROGA" +scSch + "KA SLATINA", "15.6439", "46.2409", "A", 289, 1, 2016, 0, 0);
addUndefTempPlaceDataPeriod(cv_placeRogaskaSlatina, 1, 2010, 12, 2015); //tu vmes ni podatkov, vseeno pa ohranim eno lokacijo, le graf se tu vmes ne bo prikazoval

// ŠKOCJAN  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeSkocjan = addPlace(scSch + "kocjan (420m)", scSch + "kocjan", scSch + "KOC", scSch + "kocjan", "darkTurquoise", "white");
station = addStation(cv_placeSkocjan, 2216, scSch + "KOCJAN", "13.9931", "45.6638", "A", 420, 9, 2004, 0, 0);

// LESCE  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeLesce = addPlace("Lesce letali" + scSchLow + scTchLow + "e (509m)", "Lesce letali" + scSchLow + scTchLow + "e", "LESCE", "Lesce", "darkViolet", "white");
station = addStation(cv_placeLesce, 2213, "LESCE - LETALI" + scSch + scTch + "E", "14.1718", "46.3620", "A", 509, 4, 2004, 0, 0);

// CERKLJE OB KRKI  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeCerkljeObKrki = addPlace("Cerklje letali" + scSchLow + scTchLow + "e (154m)", "Cerklje letali" + scSchLow + scTchLow + "e", "CERKLJE", "Cerklje ob Krki", "darkRed", "white");
station = addStation(cv_placeCerkljeObKrki, 3408, "CERKLJE - LETALI" + scSch + scTch + "E", "15.5184", "45.8987", "A", 154, 8, 2004, 5, 2014);
station = addStation(cv_placeCerkljeObKrki, 2215, "CERKLJE - LETALI" + scSch + scTch + "E", "15.5161", "45.9010", "A", 154, 7, 2014, 0, 0);

// ROGLA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeRogla = addPlace("Rogla (1495m)", "Rogla", "ROGLA", "Rogla", "forestGreen", "white");
station = addStation(cv_placeRogla, 1848, "ROGLA", "15.3315", "46.4530", "A", 1495, 4, 1995, 0, 0);

// GORNJI GRAD  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeGornjiGrad = addPlace("Gornji Grad (438m)", "Gornji Grad", "GGRAD", "Gornji Grad", "mediumBlue", "white");
station = addStation(cv_placeGornjiGrad, 2632, "GORNJI GRAD", "14.8063", "46.2987", "A", 438, 10, 2015, 0, 0);

// JERUZALEM  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 30.12.2023  
const cv_placeJeruzalem = addPlace("Jeruzalem (334m)", "Jeruzalem", "JRZLM", "Jeruzalem", "paleVioletRed", "white");
station = addStation(cv_placeJeruzalem, 2638, "JERUZALEM", "16.1880", "46.4759", "A", 334, 3, 2016, 0, 0);

// TREBNJE  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 31.12.2023  
const cv_placeTrebnje = addPlace("Trebnje (303m)", "Trebnje", "TREBNJE", "Trebnje", "cornFlowerBlue", "white");
station = addStation(cv_placeTrebnje, 2843, "TREBNJE", "15.0072", "45.9110", "A", 303, 8, 2015, 0, 0);

// MEŽICA  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 31.12.2023  
const cv_placeMezica = addPlace("Me" + scZhLow + "ica (469m)", "Me" + scZhLow + "ica", "ME" + scZh + "ICA", "Me" + scZhLow + "ica", "darkKhaki", "white");
station = addStation(cv_placeMezica, 2654, "ME" + scZh + "ICA", "14.8596", "46.5296", "A", 469, 7, 2016, 0, 0);

// JADRAN  lokacije in ARSO-Id postaj glej spodaj med podatki  ... 17.7.2024
// https://vode.arso.gov.si/hidarhiv/pov_arhiv_tab.php?p_vodotok=Jadransko%20morje
const cv_placeMorjeKoper = addPlace("Morje Koper (0m)", "Jadran", "JADRAN", "Morje Koper", "blue", "white");
station = addStation(cv_placeMorjeKoper, 3095, "JADRAN", "14.8596", "46.5296", "A", 469, 7, 2016, 0, 0);
addUndefTempPlaceDataPeriod(cv_placeMorjeKoper, 1, 1992, 12, 2005); //tu vmes ni podatkov, vseeno pa ohranim eno lokacijo, le graf se tu vmes ne bo prikazoval
...
addAvgTempYear(cv_placeSkofjaLoka, 2024, 1, [0.5, 5.6, 8.3, 11.1, 15.1, 19.5, 22.6, 22.9, 16, 12, 3.8, 1.2]);
addAvgTempYear(cv_placeSkofjaLoka, 2025, 1, [2.2, 2.4, 7.4, 12, 14.3, 22.1, 20.6]);
...
addAvgTempYear(cv_placeMariborVrbanskiPlato, 2024, 1, [1.1, 8, 9.2, 11.9, 15.6, 20.5, 22.4, 22.8, 16, 12.1, 4.1, 2.6]);
addAvgTempYear(cv_placeMariborVrbanskiPlato, 2025, 1, [3.1, 0.8, 8, 12.2, 14.2, 21.9, 20.9, , , , ,]);
...
addAvgTempYear(cv_placeLjubljanaBezigrad, 2024, 1, [1.6, 7.6, 9.7, 12.7, 16.2, 21.2, 24.4, 24.4, 17.2, 13.2, 5.4, 2]);
addAvgTempYear(cv_placeLjubljanaBezigrad, 2025, 1, [4, 3.4, 8.6, 13.5, 15.5, 24.3, 22.4, , , , ,]);
...
addAvgTempYear(cv_placeNovoMesto, 2024, 1, [1.6, 8.2, 9.8, 12.6, 16, 21.1, 23.9, 23.8, 16.7, 12.9, 5, 2.3]);
addAvgTempYear(cv_placeNovoMesto, 2025, 1, [3.9, 2.4, 8.2, 12.9, 15, 23.5, 21.6, , , , ,]);
...
addAvgTempYear(cv_placePostojna, 2024, 1, [2.3, 6.6, 7.8, 10.4, 13.8, 18.6, 22.1, 22.1, 14.9, 12.9, 5, 2.3]);
addAvgTempYear(cv_placePostojna, 2025, 1, [3.5, 2.3, 6.9, 10.5, 13.1, 21.2, 19.7, , , , ,]);
...
addAvgTempYear(cv_placeNovaGoricaBilje, 2024, 1, [3.8, 8.1, 10.3, 13.1, 17.2, 21.5, 25.4, 25.7, 19, 15.3, 7.5, 4.4]);
addAvgTempYear(cv_placeNovaGoricaBilje, 2025, 1, [6.7, 6.3, 9.7, 14, 16.7, 24.6, 23, , , , ,]);
...
addAvgTempYear(cv_placeKredarica, 2024, 1, [-5.4, -3.1, -4, -1.2, 1.3, 6.7, 9.7, 11.1, 3.1, 3.1, -1.4, -5]);
addAvgTempYear(cv_placeKredarica, 2025, 1, [-6, -5.3, -4, -1.4, 0.7, 9.4, 7, , , , ,]);
...
addAvgTempYear(cv_placeSlovenjGradecSmartno, 2024, 1, [-1.2, 4.7, 7.8, 10.9, 14.3, 19.5, 21.6, 21.7, 14.9, 11.8, 2.9, 0.1]);
addAvgTempYear(cv_placeSlovenjGradecSmartno, 2025, 1, [2, 1.3, 6.5, 11, 13.7, 21.7, 19.9, , , , ,]);
...
addAvgTempYear(cv_placeKrvavec, 2024, 1, [-1.8, 1.1, 0.9, 3.9, 6.5, 11.6, 14.6, 16.4, 8.4, 7, 2, -0.6]);
addAvgTempYear(cv_placeKrvavec, 2025, 1, [-2.3, -2.9, 0.6, 3.7, 5.5, 13.8, 12.4, , , , ,]);
...
addAvgTempYear(cv_placeMurskaSobota, 2024, 1, [0.3, 7.3, 9.2, 12.8, 16.5, 21.3, 23.5, 23.5, 16.5, 12.4, 3.8, 1.6]);
addAvgTempYear(cv_placeMurskaSobota, 2025, 1, [3.2, 1.1, 8, 12.9, 15, 22.5, 21.3, , , , ,]);
...
addAvgTempYear(cv_placeCelje, 2024, 1, [0.1, 6.8, 8.8, 11.6, 15.4, 20.7, 22.4, 22.7, 15.9, 12.6, 4.4, 1.7]);
addAvgTempYear(cv_placeCelje, 2025, 1, [3.8, 1.8, 7.5, 11.9, 14.7, 22.5, 20.9, , , , ,]);
...
addAvgTempYear(cv_placeBabnoPolje, 2024, 1, [-1.3, 3.7, 5.5, 7.9, 12.2, 16.7, 19.7, 19.5, 12.3, 9.6, 2.3, -2]);
addAvgTempYear(cv_placeBabnoPolje, 2025, 1, [0.7, -0.7, 3.6, 7.6, 10.6, 18.9, 17.2, , , , ,]);
...
addAvgTempYear(cv_placePortorozLetalisce, 2024, 1, [5.6, 9, 11.3, 13.6, 17.8, 22.5, 26, 26.2, 19.6, 16.4, 9, 6.4]);
addAvgTempYear(cv_placePortorozLetalisce, 2025, 1, [7.7, 6.4, 10.1, 13.8, 17.1, 24.8, 23.9, , , , ,]);
...
addAvgTempYear(cv_placeKocevje, 2024, 1, [0.7, 6.2, 7.8, 10.3, 14, 19.2, 21.8, 21.7, 14.6, 11.3, 3.9, 0.2]);
addAvgTempYear(cv_placeKocevje, 2025, 1, [2.8, 0.9, 6.2, 10.4, 12.6, 20.3, 18.9, , , , ,]);
...
addAvgTempYear(cv_placeRatece, 2024, 1, [-2.5, 2.4, 5.1, 7.9, 11.8, 16.5, 19.6, 19.4, 12.1, 9.2, 0.9, -2.4]);
addAvgTempYear(cv_placeRatece, 2025, 1, [-0.6, -0.3, 3.7, 9, 11.5, 19.8, 17, , , , ,]);
...
addAvgTempYear(cv_placeVojsko, 2024, 1, [-0.3, 4.6, 4.6, 7.7, 11.4, 15.8, 19.2, 19.9, 12.2, 9.2, 2.3, -0.7]);
addAvgTempYear(cv_placeVojsko, 2025, 1, [0.2, -1.4, 4, 8.2, 10.1, 18, 17, , , , ,]);
...
addAvgTempYear(cv_placeBrnikLetalisce, 2024, 1, [-1, 4.9, 7.9, 10.6, 14.8, 19.6, 22.2, 22.3, 15.7, 11.8, 3.2, 0.7]);
addAvgTempYear(cv_placeBrnikLetalisce, 2025, 1, [2.2, 1.8, 6.4, 11.7, 14.1, 22.2, 20.4, , , , ,]);
...
addAvgTempYear(cv_placeBizeljsko, 2024, 1, [0.6, 8.1, 9.8, 12.6, 16.6, 21.4, 23.8, 23.9, 16.8, 12.8, 4.8, 2.2]); // sep?? 21.6 ali 16.8?
addAvgTempYear(cv_placeBizeljsko, 2025, 1, [3, 2.4, 8.7, 13, 15, 23.1, 21.7, , , , ,]);
...
addAvgTempYear(cv_placeMariborLetalisce, 2024, 1, [0.5, 7.3, 9.2, 12.3, 16.2, 21.1, 23, 22.9, 16.7, 12.8, 4.1, 2]);
addAvgTempYear(cv_placeMariborLetalisce, 2025, 1, [3.3, 1.3, 8.2, 12.5, 14.8, 22.6, 21.5, , , , ,]);
...
addAvgTempYear(cv_placeLogatec, 2024, 1, [0.3, 5.5, 7, 9.6, 13.6, 18.3, 21, 21.4, 14.6, 11.4, 3.8, -0.6]); // 4464 4033 4464 4320 4464 4320 4464 4464 4320
addAvgTempYear(cv_placeLogatec, 2025, 1, [2.8, 0.7, 6.1, 10.0, 12.7, 20.3, 19]); // 4464 4032 4464 4320 4464 4320 145 48 48 48 48 48
...
addAvgTempYear(cv_placeTolmin, 2024, 1, [2.5, 6.8, 9.5, 12.3, 15.6, 20.1, 23.7, 24.5, 16.9, 13.6, 5.9, 2.3]); // 4464 4033 4464 4320 4464 4320 4464 4464
addAvgTempYear(cv_placeTolmin, 2025, 1, [4.5, 4.9, 8.7, 13.2, 15.1, 23.0, 21.1]); // 4464 4032 4464 4320 4464 4320 145 48 48 48 48 48
...
addAvgTempYear(cv_placeVelenje, 2024, 1, [2.1, 7.4, 9.6, 12.5, 16.1, 20.9, 23.4, 24, 17, 13.1, 5.1, 3.3]); // 1488 1345 1488 1440 1488 1440 1488 1488
addAvgTempYear(cv_placeVelenje, 2025, 1, [3.7, 2.5, 8.4, 12.7, 15.1, 23.2, 21.9]); // 1488 1344 1488 1440 1488 1440 49 48 48 48 48 48
...
addAvgTempYear(cv_placePtuj, 2024, 1, [0.6, 7.6, 9.3, 12.5, 16.2, 20.8, 23, 23.3, 16.5, 12.6, 4.5, 1.6]); // 4464 4033 4464 4320 4464 4320 4464 4464
addAvgTempYear(cv_placePtuj, 2025, 1, [3.5, 1.3, 8.2, 12.4, 14.6, 22.2, 21.2]); // 4464 4032 4464 4320 4464 4320 145 48 48 48 48 48
...
addAvgTempYear(cv_placeAjdovscina, 2024, 1, [4.8, 8.4, 10.7, 13.5, 17.3, 21.2, 25.3, 25.8, 18.7, 15.6, 9.1, 5.4]); // 1488 1345 1488 1440 1488 1440 1488 1488
addAvgTempYear(cv_placeAjdovscina, 2025, 1, [6.7, 6.5, 10.3, 14.0, 16.1, 24.2, 22.5]); // 1488 1344 1488 1440 1488 1440 49 48 48 48 48 48
...
addAvgTempYear(cv_placeCrnomelj, 2024, 1, [1.9, 8.5, 10, 12.3, 16.4, 21.2, 23.4, 24.1, 17.1, 12.9, 5.6, 2.2]);
addAvgTempYear(cv_placeCrnomelj, 2025, 1, [4.1, 2.7, 8.9, 12.4, 15.3, 23.1, 22.2, , , , ,]);
...
addAvgTempYear(cv_placeGodnje, 2024, 1, [4.6, 8.3, 9.7, 12.3, 15.9, 20.2, 24.5, 24.7, 17.7, 14.2, 7.9, 5.2]);
addAvgTempYear(cv_placeGodnje, 2025, 1, [5.8, 4.9, 9.1, 12.8, 15.1, 23.3, 21.9, , , , ,]);
...
addAvgTempYear(cv_placeBovec, 2024, 1, [1.3, 5.4, 8.2, 11.2, 14, 18.7, 22.2, 23.3, 16.2, 12.0, 3.8, 1.6]); // 1488 1345 1488 1440 1488 1440 1488 1488
); // 4464 4032 4464 4320 4464 4320 4464 4464 4320 4464 4320 4464
addAvgTempYear(cv_placeMezica, 2019, 1, [-1.8, 1.6, 5.8, 9.4, 10.5, 20.2, 19.8, 19.4, 14.3, 10.0, 6.5, 0.3]); // 4464 4032 4464 4320 4464 4320 4464 4464 4320 4464 4320 4464
addAvgTempYear(cv_placeMezica, 2020, 1, [-1.4, 2.7, 4.4, 10.1, 12.8, 16.7, 18.7, 19.3, 14.7, 8.6, 2.7, 0.5]); // 4464 4176 4464 4320 4464 4320 4464 4464 4320 4464 4320 4464
...
addAvgTempYear(cv_placeMezica, 2024, 1, [-1.8, 3.8, 7.2, 10.2, 14.1, 18.6, 21.3, 21.7, 14.7, 11.1, 2.2, -0.4]); // 4464 4033 4464 4320 4464 4320 4464 4464
addAvgTempYear(cv_placeMezica, 2025, 1, [0.2, 0.9, 6.0, 10.8, 13.3, 21.5, 19.7]); // 4464 4032 4464 4320 4464 4320 145 48 48 48 48 48

// 15.7.2024 MORJE
// https://vode.arso.gov.si/hidarhiv/pov_arhiv_tab.php?p_vodotok=Jadransko%20morje
addAvgTempYear(cv_placeMorjeKoper, 1957, 7, [25.4, 23.7, 21.4, 17.9, 15.2, 11.1]);
addAvgTempYear(cv_placeMorjeKoper, 1958, 1, [9.1, 8.7, 8.7, 10.5, 18.9, 22.0, 24.5, 25.2, 22.4, 19.5, 14.9, 12.0]);
addAvgTempYear(cv_placeMorjeKoper, 1959, 1, [9.5, 8.6, 10.8, 13.2, 16.5, 21.0, 24.7, 23.7, 20.0, 16.8, 13.2, 11.9]);
addAvgTempYear(cv_placeMorjeKoper, 1960, 1, [8.8, 8.5, 10.0, 12.6, 17.2, 23.1, 23.4, 24.4, 21.0, 18.5, 16.0, 13.3]);
addAvgTempYear(cv_placeMorjeKoper, 1961, 1, [9.8, 9.6, 11.3, 14.6, 17.5, 22.9, 23.8, 23.7, 22.9, 19.8, 15.6, 10.8]);
addAvgTempYear(cv_placeMorjeKoper, 1962, 1, [9.8, 7.6, 7.5, 11.4, 16.3, 19.4, 22.1, 24.6, 22.2, 18.5, 14.9, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1963, 1, [6.9, 5.5, 6.7, 11.6, 16.6, 22.1, 24.3, 25.5, 22.7, 18.4, 15.4, 11.5]);
addAvgTempYear(cv_placeMorjeKoper, 1964, 1, [8.3, 7.2, 8.6, 13.1, 17.5, 21.3, 23.8, 23.5, 21.3, 17.9, 14.1, 11.7]);
addAvgTempYear(cv_placeMorjeKoper, 1965, 1, [10.0, 7.5, 9.0, 11.4, 16.6, 20.8, 24.3, 23.7, 21.8, 18.9, 15.2, 12.3]);
addAvgTempYear(cv_placeMorjeKoper, 1966, 1, [8.6, 9.0, 9.6, 13.4, 17.5, 23.3, 24.4, 23.7, 22.1, 20.6, 15.2, 11.9]);
addAvgTempYear(cv_placeMorjeKoper, 1967, 1, [8.7, 8.0, 10.0, 11.8, 17.0, 20.7, 23.5, 24.2, 22.9, 20.4, 16.3, 10.9]);
addAvgTempYear(cv_placeMorjeKoper, 1968, 1, [7.8, 8.4, 9.5, 15.2, 16.9, 19.6, 24.6, 23.5, 21.8, 19.2, 15.9, 11.5]);
addAvgTempYear(cv_placeMorjeKoper, 1969, 1, [8.5, 7.6, 9.1, 11.4, 19.2, 22.1, 24.1, 23.5, 21.5, 19.4, 15.9, 9.8]);
addAvgTempYear(cv_placeMorjeKoper, 1970, 1, [9.8, 8.6, 8.7, 11.6, 15.6, 22.9, 22.3, 24.3, 22.4, 18.0, 14.4, 11.3]);
addAvgTempYear(cv_placeMorjeKoper, 1971, 1, [8.6, 8.7, 7.5, 11.9, 19.0, 21.9, 23.8, 25.0, 20.8, 17.1, 14.0, 10.6]);
addAvgTempYear(cv_placeMorjeKoper, 1972, 1, [8.7, 8.8, 10.3, 13.1, 16.6, 21.8, 22.5, 23.6, 19.9, 16.0, 13.8, 11.4]);
addAvgTempYear(cv_placeMorjeKoper, 1973, 1, [8.8, 8.2, 8.3, 10.4, 16.0, 21.8, 24.6, 24.0, 22.4, 18.3, 14.0, 9.8]);
addAvgTempYear(cv_placeMorjeKoper, 1974, 1, [9.4, 9.5, 10.4, 12.4, 17.0, 19.9, 23.5, 25.0, 22.7, 16.7, 13.4, 11.2]);
addAvgTempYear(cv_placeMorjeKoper, 1975, 1, [9.4, 9.0, 9.7, 12.0, 18.1, 21.3, 23.7, 23.5, 22.9, 18.9, 14.9, 11.7]);
addAvgTempYear(cv_placeMorjeKoper, 1976, 1, [9.6, 8.7, 8.6, 11.5, 17.2, 20.5, 24.0, 22.2, 20.7, 19.3, 15.4, 11.6]);
addAvgTempYear(cv_placeMorjeKoper, 1977, 1, [10.0, 9.5, 11.6, 12.9, 17.6, 21.9, 24.1, 23.9, 20.8, 17.2, 15.2, 10.8]);
addAvgTempYear(cv_placeMorjeKoper, 1978, 1, [9.2, 8.4, 9.6, 11.5, 15.4, 20.6, 23.0, 23.8, 21.0, 18.1, 14.4, 10.3]);
addAvgTempYear(cv_placeMorjeKoper, 1979, 1, [8.6, 8.4, 9.5, 12.1, 16.9, 23.0, 23.1, 23.9, 21.8, 18.6, 14.1, 11.7]);
addAvgTempYear(cv_placeMorjeKoper, 1980, 1, [9.0, 9.2, 9.6, 11.7, 14.6, 20.2, 22.9, 24.9, 21.9, 18.9, 13.9, 9.5]);
addAvgTempYear(cv_placeMorjeKoper, 1981, 1, [7.6, 6.4, 8.5, 12.2, 16.8, 21.5, 23.4, 23.2, 21.5, 19.4, 14.8, 10.8]);
addAvgTempYear(cv_placeMorjeKoper, 1982, 1, [8.9, 7.8, 8.1, 11.0, 16.0, 22.0, 24.5, 24.6, 23.3, 19.4, 15.3, 12.6]);
addAvgTempYear(cv_placeMorjeKoper, 1983, 1, [9.4, 7.9, 8.9, 12.8, 18.2, 20.2, 24.6, 24.1, 22.4, 19.1, 14.5, 9.8]);
addAvgTempYear(cv_placeMorjeKoper, 1984, 1, [8.3, 7.5, 8.1, 10.6, 14.3, 20.2, 22.7, 23.8, 21.9, 18.9, 16.0, 12.4]);
addAvgTempYear(cv_placeMorjeKoper, 1985, 1, [8.2, 7.6, 8.6, 11.7, 16.4, 20.9, 23.8, 24.8, 22.3, 19.4, 14.3, 11.1]);
addAvgTempYear(cv_placeMorjeKoper, 1986, 1, [9.4, 7.4, 8.6, 11.8, 18.9, 20.7, 23.1, 24.6, 21.3, 18.9, 14.6, 11.5]);
addAvgTempYear(cv_placeMorjeKoper, 1987, 1, [8.0, 7.9, 7.4, 11.1, 16.3, 20.8, 24.1, 24.1, 24.0, 19.9, 16.0, 12.3]);
addAvgTempYear(cv_placeMorjeKoper, 1988, 1, [10.7, 9.9, 9.7, 12.8, 16.9, 21.6, 24.5, 24.3, 21.6, 19.4, 14.7, 10.5]);
addAvgTempYear(cv_placeMorjeKoper, 1989, 1, [8.3, 8.4, 9.9, 13.8, 16.3, 20.9, 24.4, 23.8, 20.8, 18.5, 14.8, 10.2]);
addAvgTempYear(cv_placeMorjeKoper, 1990, 1, [8.1, 9.1, 10.6, 12.1, 19.3, 20.7, 23.3, 22.9, 21.2, 19.6, 15.6, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1991, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
//addAvgTempYear(cv_placeMorjeKoper, 1992, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1993, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1994, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1995, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1996, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1997, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1998, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 1999, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2000, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2001, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2002, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2003, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2004, 1, [, , , , , , , , , , , ]);
//addAvgTempYear(cv_placeMorjeKoper, 2005, 1, [, , , , , , , , , , ,]);
addAvgTempYear(cv_placeMorjeKoper, 1992, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1993, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1994, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1995, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1996, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1997, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1998, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 1999, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2000, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2001, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2002, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2003, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2004, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2005, 1, [7.5, 6.6, 9.2, 10.5, 14.1, 19.3, 23.7, 24.4, 21.4, 18.3, 14.5, 9.7]);
addAvgTempYear(cv_placeMorjeKoper, 2006, 1, [8.5, 7.8, 8.3, 12.5, 17.6, 21.6, 24.5, 23.4, 23.1, 20.3, 16.4, 13.1]);
addAvgTempYear(cv_placeMorjeKoper, 2007, 1, [11.3, 11.0, 11.9, 15.0, 20.2, 23.2, 25.2, 24.5, 21.7, 18.4, 13.4, 10.9]);
addAvgTempYear(cv_placeMorjeKoper, 2008, 1, [9.5, 8.8, 9.9, 12.8, 18.4, 22.1, 24.6, 24.5, 21.4, 17.7, 15.1, 11.9]);
addAvgTempYear(cv_placeMorjeKoper, 2009, 1, [9.4, 9.3, 9.7, 13.6, 19.7, 21.4, 24.9, 26.2, 23.1, 19.4, 15.1, 11.9]);
addAvgTempYear(cv_placeMorjeKoper, 2010, 1, [9.8, 8.7, 9.8, 13.8, 18.6, 23.5, 25.0, 24.0, 21.3, 18.3, 14.9, 12.2]);
addAvgTempYear(cv_placeMorjeKoper, 2011, 1, [9.0, 8.5, 9.3, 13.1, 16.8, 21.1, 24.3, 25.0, 24.2, 18.5, 14.9, 12.3]);
addAvgTempYear(cv_placeMorjeKoper, 2012, 1, [9.8, 5.8, 9.5, 12.3, 16.8, 22.7, 25.4, 25.6, 22.3, 20.0, 16.4, 12.2]);
addAvgTempYear(cv_placeMorjeKoper, 2013, 1, [10.5, 9.1, 9.9, 13.4, 18.3, 22.3, 24.4, 24.1, 22.5, 18.7, 16.1, 11.1]);
addAvgTempYear(cv_placeMorjeKoper, 2014, 1, [10.8, 10.7, 11.9, 14.5, 17.7, 21.7, 25.1, 25.4, 22.4, 20.7, 17.5, 14.3]);
addAvgTempYear(cv_placeMorjeKoper, 2015, 1, [10.3, 9.3, 10.3, 13.2, 18.1, 22.6, 27.0, 25.5, 22.7, 18.4, 15.1, 12.0]);
addAvgTempYear(cv_placeMorjeKoper, 2016, 1, [10.4, 10.5, 11.4, 14.6, 17.7, 22.8, 26.2, 24.2, 23.4, 19.6, 15.8, 12.2]);
addAvgTempYear(cv_placeMorjeKoper, 2017, 1, [8.2, 9.0, 10.9, 13.9, 18.5, 25.0, 26.0, 25.9, 23.0, 19.5, 15.7, 12.3]);
addAvgTempYear(cv_placeMorjeKoper, 2018, 1, [11.2, 9.4, 9.0, 14.0, 20.2, 23.7, 25.7, 25.5, 24.1, 19.5, 16.5, 12.3]);
addAvgTempYear(cv_placeMorjeKoper, 2019, 1, [9.2, 9.4, 10.7, 13.4, 15.6, 23.7, 26.2, 26.2, 23.0, 20.2, 17.4, 14.3]);
addAvgTempYear(cv_placeMorjeKoper, 2020, 1, [11.3, 10.8, 11.5, 13.7, 17.4, 22.6, 23.8, 25.4, 23.6, 19.7, 16.3, 12.9]);
addAvgTempYear(cv_placeMorjeKoper, 2021, 1, [11.3, 10.1, 11.2, 12.9, 17.8, 24.1, 26.5, 25.8, 23.0, 18.2, 15.2, 12.0]);
addAvgTempYear(cv_placeMorjeKoper, 2022, 1, [9.6, 9.4, 10.1, 12.8, 18.7, 24.3, 26.5, 25.0, 23.4, 21.1, 17.4, 13.8]);
//----
addAvgWaterLevelYear(cv_placeMorjeKoper, 1957, 7, [25.4, 23.7, 21.4, 17.9, 15.2, 11.1]);
...
const arrToolTipPlace = [];     // 26.12.2023
const arrToolTipTimeSlice = []; // 26.12.2023
...
var lo_selectedHribi = true; // 30.12.2023
...
function paint_stations() {
    ...
    //----    
    let place, place2, station, station2, oldPlace;
    ...
    let tmpNrPlaces, tmpNrStations, tmpNrPlacesStations, checkedPlace, checkedPlaceStation, newCol, vl_overLimit, lastPlace;
    //----
    ...
    //const colStations = 13; // toliko lokacij znotaj enega stolpca. Preizkušeno deluje tudi za 8 ali poljubno drugo številko!
    const colPlacesStations = 55; // toliko lokacij+postaj znotaj enega stolpca.
    let yGapTop, tmpMaxWidth;
    let valueCorrection; //16.12.2023
    let wColShorten = 0; //17.12.2023
    //gText("Javorje", fontPlace, "black", 0, 0);
    //console.log("--------------------");
    let stationNextCol = 1;
    oldPlace = 0; y = y0; newCol = true;
    const cv_log = false;
    for (station = 1; station <= nrStations; station++) {
        if (cv_log) { console.log("station=" + station.toString()) };
        place = stationPlace[station];
        if (station == stationNextCol) {
            if (cv_log) { console.log("station == stationNextCol") }
            //---- smo na preskoku v naslednji stolpec
            newCol = true; y = y0;
            //     treba je ugotoviti ševilo lokacij in postaj v njem, njegovo višino in širino, ter X pozicijo
            if (station == 1) {
                xCol = 0;
                if (cv_log) { console.log("xCol=0") };
            } else {
                xCol += wColGap + wCol - wColShorten
                if (cv_log) { console.log("xCol=" + xCol.toString()) };
            };
            
            //---- zanka po nadaljnjih postajah in gledam, koliko jih pride v trenutni stolpec
            tmpNrPlaces = 0; tmpNrStations = 0; tmpNrPlacesStations = 0; checkedPlace = place - 1; checkedPlaceStation = station - 1;
            vl_overLimit = false; lastPlace = 0;
            for (station2 = station; station2 <= nrStations; station2++) {
                place2 = stationPlace[station2];
                if (cv_log) { cLogT2("    ", "station2", station2, "place2", place2) };
                if (place2 > lastPlace) {
                    //---- je že naslednji place - preverim, če je prejšnji z vsemi svojimi postajami ustrezal pogojem
                    tmpNrStations = station2 - station;
                    tmpNrPlaces = place2 - place;
                    tmpNrPlacesStations = tmpNrPlaces + tmpNrStations;
                    if (tmpNrPlacesStations <= colPlacesStations) {
                        //---- prejšnja lokacija s svojimi postajami vred ustreza pogoju za izpis v en stolpec, s preverjanjem postaj pa je treba nadaljevati
                        checkedPlace = place2 - 1;
                        checkedPlaceStation = station2 - 1;
                        if (cv_log) { cLogT2("    ", "checkedPlace", checkedPlace, "checkedPlaceStation", checkedPlaceStation) };
                    } else {
                        //---- prejšnja lokacija s svojimi postajami že preseže dolžino za v ta stolpec
                        //     dobra je lokacija/postaja, shranjena v checkedPlace/checkedPlaceStation
                        vl_overLimit = true;
                        if (cv_log) { console.log("    over-limit") }
                        break;
                    }
                } else {
                    //---- smo še v okviru iste lokacije, ki jo sedaj preverjamo, nadaljujemo preverjanje
                }
                lastPlace = place2;
            }
            if (!vl_overLimit) {
                //---- ni šlo čez mejo že znotraj zanke, isto je treba zdaj preveriti še skupaj z zadnjo lokacijo in njenimi postajami
                if (cv_log) { console.log("    not-over-limit") }
                tmpNrStations = nrStations - station + 1;
                tmpNrPlaces = nrPlaces - place + 1;
                tmpNrPlacesStations = tmpNrPlaces + tmpNrStations;
                if (tmpNrPlacesStations <= colPlacesStations) {
                    //---- tudi zadnja lokacija in njene postaje grejo lepo v trenuten stolpec
                    checkedPlace = nrPlaces;
                    checkedPlaceStation = nrStations;  
                    if (cv_log) { cLogT2("    ", "checkedPlace", checkedPlace, "checkedPlaceStation", checkedPlaceStation) };
                } else {
                    //---- z zadnjo lokacijo in njenimi postajami gre čez mejo za stolpec, obvelja checkedPlace/checkedPlaceStation ki je bil zadnji najden prej v zanki
                }
            }
            tmpNrStations = checkedPlaceStation - station + 1;
            tmpNrPlaces = checkedPlace - place + 1;
            //tmpLastCheckedNrPlacesStations = tmpNrPlaces + tmpNrStations;
            if (cv_log) { cLog4("checkedPlace", checkedPlace, "checkedPlaceStation", checkedPlaceStation, "tmpNrStations", tmpNrStations, "tmpNrPlaces", tmpNrPlaces) };
            tmpMaxWidth = 0;
            for (station2 = station; station2 <= checkedPlaceStation; station2++) {
            ...
            if (cv_log) { cLog1("tmpMaxWidth", tmpMaxWidth) };
            stationNextCol = checkedPlaceStation + 1;
            if (cv_log) { cLog1("stationNextCol", stationNextCol) };
            //
            wColShorten = 0;
            ...
            if (cv_log) { console.log("banner-drawn") };
        }

        //---- izpis lokacije
        if (place !== oldPlace) { //---- smo na prehodu na novo lokacijo z njenimi postajami?
            ...
            if (cv_log) { cLogT1("location printed", "place", place) };
            y += tmpH + vStepPlace;
        }
        oldPlace = place;
        ...
        //---- izpis eventualne korekcije temperatur postaje
        ...
        if (cv_log) { cLogT1("** station printed", "station", station) };
    ...
    for (station = 1; station <= nrStations; station++) {
        ...
        if (place !== stationPlace[station]) {
            ...
            switch (place) {
                ...
                case colStations: case 2 * colStations: case 3 * colStations: case 4 * colStations: xCol += wColGap + wCol - wColShorten; y = y0; newCol = true; break;
            }
            //---- lokacija se je spremenila, treba jo je izpisati
            place = stationPlace[station];            
...
function lf_changeSelectedHribi(vp_newValue, vp_paint) {
    // 30.12.2023 selektira/deselektira lokacije nad 900m višine (hribi in gore)

    lo_selectedHribi = vp_newValue;

    let place, station;
    for (station = 1; station <= nrStations; station++) {
        if (stationHeight[station] >= 700) {
            lo_enabledPlace[stationPlace[station]] = lo_selectedHribi;
        }
    }
    ...
    switch (lo_showMap) {
        case true:
            ...
            if (!lo_mapCreated) { 
                ...
                //mapOptions = { center: mapLocation, zoom: 9, mapTypeId: google.maps.MapTypeId.HYBRID };  deluje! https://www.w3schools.com/graphics/google_maps_types.asp
                gMap = new google.maps.Map(gMapDiv, mapOptions);
                //gMap.setMapTypeId(google.maps.MapTypeId.ROADMAP);  // 2D mapa z oznakami mest, cest, rek ...
                //gMap.setMapTypeId(google.maps.MapTypeId.TERRAIN); // 2D mapa z oznakami + relief
                //gMap.setMapTypeId(google.maps.MapTypeId.SATELLITE); // satelit brez oznak mest, cest, rek ...
                gMap.setMapTypeId(google.maps.MapTypeId.HYBRID); // staelit z oznakami mest, cest, rek ...
                //gMap.setTilt(45); // nekako mi ni delalo na HYBRID mode
                ...
                let station, tmpStr, myImage, myLabel;
                for (station = 1; station <= nrStations; station++) {
                    mapLocation.lat = +stationLat[station]; mapLocation.lng = +stationLon[station];
                    tmpStr = stationName[station];
                    tmpStr += " (";
                    switch (stationType[station]) {
                        case "A": tmpStr += "ARSO-samodejna postaja";
                            //myImage = 'http://maps.google.com/mapfiles/ms/icons/red-dot.png';
                            myImage = 'https://maps.google.com/mapfiles/kml/paddle/A.png';
                            break;
                        case "P": tmpStr += "Privatna postaja";
                            myImage = 'http://maps.google.com/mapfiles/ms/icons/yellow-dot.png';
                            myImage = 'http://maps.gstatic.com/mapfiles/ms2/micons/yellow-dot.png';
                            break;
                        default: tmpStr += "ARSO";
                            myImage = 'http://maps.google.com/mapfiles/ms/icons/blue-dot.png';
                            break;
                    }
                    myLabel = stationType[station]; myLabel = "";
                    tmpStr += ", " + stationHeight[station].toString() + "m";
                    tmpStr += ", " + lf_dateStrMMMsepYY(stationMonthStart[station], stationYearStart[station], ":", 0, 0, "live");
                    tmpStr += " .. " + lf_dateStrMMMsepYY(stationMonthEnd[station], stationYearEnd[station], ":", 0, 0, "live");
                    tmpStr += ")";
                    //gMarker[station] = new google.maps.Marker({ position: mapLocation, map: gMap, title: tmpStr, opacity: 0.9, label: stationType[station], icon: 'http://www.gstatic.com/mapspro/images/stock/1475-weather-fog.png' });
                    //gMarker[station] = new google.maps.Marker({ position: mapLocation, map: gMap, title: tmpStr, opacity: 0.9, label: myLabel, icon: myImage, clickable: true });
                    gMarker[station] = new google.maps.Marker({ position: mapLocation, map: gMap, title: tmpStr, opacity: 0.9, label: myLabel, clickable: true });
                    //gMarker[station] = new google.maps.Marker({ position: mapLocation, map: gMap, title: tmpStr, opacity: 0.9, label: stationType[station], icon: { path: myImage, scale: 15, }, clickable: true });
                    //gMarker[station].clickable = true;
                    //gMarker[station].opacity = 0.9;
                    //gMarker[station].icon = myImage;
                    //gMarker[station].setAttribute("icon", myImage); ...?? še enkrat probaj a dela
                    //gMarker[station]["icon"] = myImage;   .... probaj to če dela!!!
                    // object["propertie"] = value;
                    // marker.setMap(null); // brisanje markerja
                }
            }
...
function paint_graph_timeAvgTemp_multiPlace(marginLeft, marginTop, marginRight, marginBottom, vp_graphType) {
    ...
    //----
    lo_tipMultiMonth = 0;           // 26.12.2023
    lo_tipMultiPlace = cv_allPlace; // 26.12.2023
    col = 1; row = 1;
...
function paint_graph_timeAvgTemp_multiTimeSlice(marginLeft, marginTop, marginRight, marginBottom, vp_graphType) {
    ...
    //----
    lo_tipMultiMonth = 0;                   // 26.12.2023
    lo_tipMultiTimeSlice = cv_timeSliceAll; // 26.12.2023
    col = 1; row = 1;
    k = 0.15; //30.7.2023 v1.31
    let vl_timeSlice;
...
function paint_graph_timeAvgTemp(vp_left, vp_top, vp_width, vp_height, vp_graphType, vp_place, vp_timeSlice, vp_marginRight, vp_forceDataRangeY, vp_minY, vp_maxY, vp_dataRange) {
    ...
    paint_graph_timeAvgTemp_tipBeforeGraph(vp_place, vp_timeSlice, vl_monthStart, gl_monthEnd, kx, cv_graphLeftData, cv_graphY0, ky, cv_graphLeft, cv_graphRight, cv_graphTopData, cv_graphBottom);
                        ...
                        switch (vp_place) {
                            case cv_allPlace:
                                ...
                                if (lo_addMarkWidth >= 3) {
                                    ...
                                    gText(tmpStr, "normal 11pt verdana", placeNameColor, lastPlaceMarkerX[place] + 9, lastPlaceMarkerY[place] + 3);
                                } else if (lo_addMarkWidth >= 1) {
    ...
    paint_graph_timeAvgTemp_tipAfterGraph(vp_place, vp_timeSlice, vl_monthStart, gl_monthEnd, kx, cv_graphLeftData, cv_graphY0, ky);
    ...
    paint_graph_timeAvgTemp_tipContent(vp_place, vp_timeSlice, vl_monthStart, gl_monthEnd, vp_left, vp_top, cv_graphLeft, cv_graphLeftData, cv_graphRight, cv_graphBottom, kx);
...
function paint_graph_timeAvgTemp_tipContent_old(vp_place, vp_timeSlice, vp_left, vp_top, cv_graphLeft, cv_graphRight, cv_graphBottom) {
...
function paint_graph_timeAvgTemp_tipContent(vp_place, vp_timeSlice, vp_monthStart, vp_monthEnd, vp_left, vp_top, cv_graphLeft, cv_graphLeftData, cv_graphRight, cv_graphBottom, kx) {

    //prikaz tipsov mora biti vključen
    if (!lo_showToolTips) { return };

    // 25.12.2023 najprej izpišem vse shranjene toolTip-e
    let listItem, vl_tipMonth;;
    for (listItem = 1; listItem <= nrToolTips; listItem++) {
        vl_tipMonth = arrToolTipMonth[listItem];
        if (arrToolTipMode[listItem] == gl_mode && valueBetween(vl_tipMonth, vp_monthStart, vp_monthEnd) && arrToolTipPlace[listItem] == vp_place && arrToolTipTimeSlice[listItem] == vp_timeSlice) {
            paint_graph_timeAvgTemp_tipContent_timeAvgTemp(vl_tipMonth, listItem, vp_place, vp_timeSlice, vp_monthStart, vp_left, vp_top, cv_graphLeft, cv_graphLeftData, cv_graphRight, cv_graphBottom, kx);
        }
    }
    ...
    // prikaz toolTip vsebine
    paint_graph_timeAvgTemp_tipContent_timeAvgTemp(lo_tipMonth, 0, vp_place, vp_timeSlice, vp_monthStart, vp_left, vp_top, cv_graphLeft, cv_graphLeftData, cv_graphRight, cv_graphBottom, kx);
...
function paint_graph_timeAvgTemp_tipContent_timeAvgTemp(vp_tipMonth, vp_toolTipItem, vp_place, vp_timeSlice, vp_monthStart, vp_left, vp_top, cv_graphLeft, cv_graphLeftData, cv_graphRight, cv_graphBottom, kx) {
    
    //miška mora biti nad tekočim grafom
    if (vp_tipMonth <= 0) { return };
    ...
    switch (vp_place) {
        case cv_allPlace:
            ...
            if (gl_showAvgAllPlace) {
                ...
                for (place = 1; place <= nrPlaces; place++) { 
                    ...
                    //if (vp_tipMonth <= offsetPlaceMonths) { continue }
                    //if (vp_tipMonth <= offsetMonths[place]) { continue }
                    if (!valueBetween(vp_tipMonth, firstMonth[place], lastMonth[place])) { continue }
                    tipPlacesWithData += 1;
                    tmpStr = placeName[place];
                    ;[tmpW, tmpH] = gMeasureText(tmpStr, font);
        ...
        default:
            ...
            cv_heightBody = cv_heightSingle
            //offsetPlaceMonths = (12 * minYear[place] + minMonth[place]) - (12 * minYearAll + minMonthAll); //4.12.2023
            //if (vp_tipMonth <= offsetPlaceMonths) { cv_heightBody = 0 }
            if (vp_tipMonth <= offsetMonths[place]) {
                cv_heightBody = 0
            } else {
    ...
    //---- postavitev po Y-u
    let vl_mousePosY = lo_mouseMoveY; // 25.12.2023
    if (vp_toolTipItem > 0) { vl_mousePosY = arrToolTipY[vp_toolTipItem] };
    let frameTop = vl_mousePosY - frameHeight / 2; //po Y postavi tako, da je cursor na sredini toolTip-a
    if (frameTop < vp_top) {
    ...
    //---- postavitev po X-u
    let vl_mousePosX = lo_mouseMoveX; // 25.12.2023
    if (vp_toolTipItem > 0) {
        vl_mousePosX = cv_graphLeftData + kx * (vp_tipMonth - vp_monthStart);
    };
    let frameLeft = vl_mousePosX - 8 - frameWidth;                   //postavi toolTip malce levo od kurzorja miške
    if (frameLeft < cv_graphLeft) { frameLeft = vl_mousePosX + 10 }; //če levo gre ven, naj se začne na levi na začetku
    if (frameLeft + frameWidth > cv_graphRight) { frameLeft = cv_graphRight - frameWidth }; //če desno gre ven, naj gre desno ravno do roba
    ...
    switch (vp_timeSlice) {
        case cv_timeSliceWinter: case cv_timeSliceSpring: case cv_timeSliceSummer: case cv_timeSliceAutumn: case cv_timeSliceSeason:
            tmpStr = lf_seasonStrSmallYYYY(vp_tipMonth);
            break;
        default:
            tmpStr = lf_monthStrMMMYYYY(vp_tipMonth);
            break;
    }    
    tmpStr += "  (#" + vp_tipMonth.toString() + ")";
    ;[tmpW, tmpH] = gMeasureText(tmpStr, font);
    x = frameLeft + frameWidth / 2 - tmpW / 2;
    y = frameTop + 4 + tmpH;
    gText(tmpStr, font, "darkSlateGray", x, y);
    ...
    if (gl_showAvgAllPlace && vp_place == cv_allPlace) {
        ...
        gText(":", font, "darkSlateGray", cv_xDvopicje, y);
        //----
        gText(avgTempCacheAll[vp_tipMonth].toFixed(1) + scStopinj, font, "darkBlue", cv_xDvopicje + cv_gapRight, y);
    } else {
        for (place = 1; place <= nrPlaces; place++) {
            if (!lo_enabledPlace[place]) { continue };
            ...
            gText(":", font, "darkSlateGray", cv_xDvopicje, y);
            //----
            //gText(lf_getAvgValue(place, vp_tipMonth - offsetPlaceMonths, cv_nrMonthsAvgMult * lo_nrMonthsAvg).toFixed(1) + scStopinj, font, placeColor[place], cv_xDvopicje + cv_gapRight, y);
            gText(lf_getAvgValue(place, vp_timeSlice, vp_tipMonth - offsetMonths[place], cv_nrMonthsAvgMult * lo_nrMonthsAvg).toFixed(1) + scStopinj, font, placeColor[place], cv_xDvopicje + cv_gapRight, y);
            //----
            y += cv_heightSingle;
        }
    }
...
function paint_graph_timeAvgTemp_tipBeforeGraph(vp_place, vp_timeSlice, vp_monthStart, vp_monthEnd, kx, cv_graphLeftData, cv_graphY0, ky, cv_graphLeft, cv_graphRight, cv_graphTopData, cv_graphBottom) {
    ...
    //---- določanje tipMonth 
    if (pointInsideRect(lo_mouseMoveX, lo_mouseMoveY, cv_graphLeft, cv_graphTopData, cv_graphRight, cv_graphBottom)) {
        switch (gl_mode) {
            case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiPlace: case cv_mode_timeAvgTempMultiTimeSlice:
                lo_tipMonth = Math.round(vp_monthStart + (lo_mouseMoveX - cv_graphLeftData) / kx);
                if (lo_tipMonth < vp_monthStart) { lo_tipMonth = vp_monthStart };
                if (lo_tipMonth > gl_monthEnd) { lo_tipMonth = gl_monthEnd };
                switch (vp_timeSlice) { //10.12.2023
                    case cv_timeSliceAll: break; //toolTip je lahko na vsakem mesecu
                    case 1: case 2: case 3: case 4: case 5: case 6: case 7: case 8: case 9: case 10: case 11: case 12:
                        //---- toolTip je lahko samo na mesecu, ki je določen z vp_timeSlice
                        lo_tipMonth = lf_getNearestTimeSliceMonth(lo_tipMonth, vp_timeSlice - cv_timeSliceMonthMin + 1, vp_monthStart, gl_monthEnd); //11.12.2023
                        //if (lo_tipMonth == 0) { return };
                        break;
                    case cv_timeSliceWinter: case cv_timeSliceSpring: case cv_timeSliceSummer: case cv_timeSliceAutumn:
                        //---- povprečju dodajamo le na zadnjem mesecu ustreznega letnega časa, dodamo mu pa povprečje zadnjih treh mesecev
                        tmpMonthValue2 = 2 + 3 * (vp_timeSlice - cv_timeSliceWinter);   //mora priti 2 za zimo, 5 za pomlad, 8 za poletje in 11 za jesen, primer za poletje: 2+3*(15-13)=8
                        lo_tipMonth = lf_getNearestTimeSliceMonth(lo_tipMonth, tmpMonthValue2, vp_monthStart, gl_monthEnd); //11.12.2023
                        //if (lo_tipMonth == 0) { return };
                        break;
                }
                if (lo_tipMonth > 0) {
    ...
    //---- najprej izpišem vse shranjene toolTip-e (25.12.2023)
    let listItem, vl_tipMonth;
    switch (gl_mode) {
        case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiPlace: case cv_mode_timeAvgTempMultiTimeSlice:
            // ---- izris za vse shranjene toolTip-e
            for (listItem = 1; listItem <= nrToolTips; listItem++) {
                vl_tipMonth = arrToolTipMonth[listItem];
                if (arrToolTipMode[listItem] == gl_mode && valueBetween(vl_tipMonth, vp_monthStart, vp_monthEnd) && arrToolTipPlace[listItem] == vp_place && arrToolTipTimeSlice[listItem] == vp_timeSlice) {
                    paint_graph_timeAvgTemp_tipBeforeGraph_listItem(vl_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky); // 25.12.2023  
                    //---- risanje vertikalne linije za ta toolTip
                    paint_graph_timeExcessDeath_tipVerticalLine(vl_tipMonth, vp_monthStart, kx, cv_graphLeftData, cv_graphTopData, cv_graphBottom);
                }
            }
            // ---- nato še za trenutnega pod miško
            if (lo_mouseOut) { return }; //29.7.2023 če je miška izven okna, se toolTip-i ne rišejo
            if (lo_tipMonth > 0) {
                paint_graph_timeAvgTemp_tipBeforeGraph_listItem(lo_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky); // 25.12.2023
                //---- risanje vertikalne linije za toolTip
                paint_graph_timeExcessDeath_tipVerticalLine(lo_tipMonth, vp_monthStart, kx, cv_graphLeftData, cv_graphTopData, cv_graphBottom);
            }
    }   
}

function paint_graph_timeAvgTemp_tipBeforeGraph_listItem(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky) {
    ...
    switch (gl_mode) {
        case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiTimeSlice:
            if (gl_showAvgAllPlace) {
                paint_graph_timeAvgTemp_tipMarkerDown(vp_tipMonth, cv_allPlace, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
            } else {
                for (place = 1; place <= nrPlaces; place++) {
                    paint_graph_timeAvgTemp_tipMarkerDown(vp_tipMonth, place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
                }
            }
            break;
        case cv_mode_timeAvgTempMultiPlace:
            paint_graph_timeAvgTemp_tipMarkerDown(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
            break;
    }

}

function paint_graph_timeAvgTemp_tipAfterGraph(vp_place, vp_timeSlice, vp_monthStart, vp_monthEnd, kx, cv_graphLeftData, cv_graphY0, ky) {

    //prikaz tipsov mora biti vključen
    if (!lo_showToolTips) { return };

    // 25.12.2023 najprej risanje foreground krogcev za vse shranjene toolTip-e
    let listItem, vl_tipMonth;
    for (listItem = 1; listItem <= nrToolTips; listItem++) {
        vl_tipMonth = arrToolTipMonth[listItem];
        if (arrToolTipMode[listItem] == gl_mode && valueBetween(vl_tipMonth, vp_monthStart, vp_monthEnd) && arrToolTipPlace[listItem] == vp_place && arrToolTipTimeSlice[listItem] == vp_timeSlice) { 
            paint_graph_timeAvgTemp_tipAfterGraph_listItem(vl_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky); // 25.12.2023    
        }
        
    }
    // nato pa še trenutnega pod miško
    if (lo_mouseOut) { return }; //29.7.2023 če je miška izven okna, se toolTip-i ne rišejo
    if (lo_tipMonth > 0) { //miška mora biti nad tekočim grafom
        paint_graph_timeAvgTemp_tipAfterGraph_listItem(lo_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky); // 25.12.2023
    }
}

function paint_graph_timeAvgTemp_tipAfterGraph_listItem(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky) {

    //---- risanje foreground toolTip krogcev
    let place;
    switch (gl_mode) {
        case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiTimeSlice:
            //grem čez vse lokacije in pri vsaki za vp_tipMonth narišem foreground marker
            if (gl_showAvgAllPlace) { // 24.12.2023
                paint_graph_timeAvgTemp_tipMarkerUp(vp_tipMonth, cv_allPlace, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
            } else {
                for (place = 1; place <= nrPlaces; place++) {
                    paint_graph_timeAvgTemp_tipMarkerUp(vp_tipMonth, place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
                }
            }
            break;
        case cv_mode_timeAvgTempMultiPlace:      
            //za to lokacijo za vp_tipMonth narišem foreground marker
            paint_graph_timeAvgTemp_tipMarkerUp(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky);
            break;
    }
}

function paint_graph_timeAvgTemp_tipMarkerDown(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky) {
    ...
    if (!gl_showAvgAllPlace) { //24.12.2023
        ...
        if (vp_tipMonth <= offsetMonths[vp_place]) { return }
    }
    
    //---- izračun koordinat tekočega data point-a
    let x, y, yValue, color;
    x = cv_graphLeftData + kx * (vp_tipMonth - vp_monthStart);
    switch (gl_mode) {
        case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiTimeSlice:
            if (gl_showAvgAllPlace) {
                yValue = avgTempCacheAll[vp_tipMonth]; // 24.12.2023
                color = "darkBlue";
            } else {
                yValue = lf_getAvgValue(vp_place, vp_timeSlice, vp_tipMonth - offsetMonths[vp_place], cv_nrMonthsAvgMult * lo_nrMonthsAvg)
...
function paint_graph_timeAvgTemp_tipMarkerUp(vp_tipMonth, vp_place, vp_timeSlice, vp_monthStart, kx, cv_graphLeftData, cv_graphY0, ky) {
    ...
    switch (gl_mode) {
        case cv_mode_timeAvgTempSingle: case cv_mode_timeAvgTempMultiTimeSlice:
            if (gl_showAvgAllPlace) {
                yValue = avgTempCacheAll[vp_tipMonth];
                color = "darkBlue";
            } else {
                yValue = lf_getAvgValue(vp_place, vp_timeSlice, vp_tipMonth - offsetMonths[vp_place], cv_nrMonthsAvgMult * lo_nrMonthsAvg)
                color = placeColor[vp_place];
...
function paint_graph_timeExcessDeath_tipVerticalLine(vp_tipMonth, vl_monthStart, kx, cv_graphLeftData, cv_graphTopData, cv_graphBottom) {
    
    //prikaz tipsov mora biti vključen
    if (!lo_showToolTips) { return };
    
    //risanje background krogca za to lokacijo in mesec
    let x = cv_graphLeftData + kx * (vp_tipMonth - vl_monthStart);
    gLine(x, cv_graphTopData, x, cv_graphBottom, 1, "darkGray", [3, 1]);
    ...
    for (listIndex = 1; listIndex <= nrToolTips; listIndex++) {
        ...
        if (vp_kx * Math.abs(arrToolTipMonth[listIndex] - vp_month) <= cv_diffX && arrToolTipMode[listIndex] == gl_mode && arrToolTipPlace[listIndex] == vp_place && arrToolTipTimeSlice[listIndex] == vp_timeSlice) {
            ...
            if (listIndex !== nrToolTips) {
                for (listIndex2 = listIndex + 1; listIndex2 <= nrToolTips; listIndex2++) {
                    arrToolTipMonth[listIndex2 - 1] = arrToolTipMonth[listIndex2];
                    arrToolTipMode[listIndex2 - 1] = arrToolTipMode[listIndex2];
                    arrToolTipPlace[listIndex2 - 1] = arrToolTipPlace[listIndex2];
                    arrToolTipTimeSlice[listIndex2 - 1] = arrToolTipTimeSlice[listIndex2];
                    arrToolTipY[listIndex2 - 1] = arrToolTipY[listIndex2];
                }
            }
            //---- v tabeli bo zdaj eden manj
            nrToolTips -= 1;
            return;
        };
    }

    //---- ta mesec še ni v seznamu toolTip mesecev, zato ga dodamo
    nrToolTips += 1;
    arrToolTipMonth[nrToolTips] = vp_month;
    arrToolTipMode[nrToolTips] = gl_mode;
    arrToolTipPlace[nrToolTips] = vp_place;
    arrToolTipTimeSlice[nrToolTips] = vp_timeSlice;
    arrToolTipY[nrToolTips] = vp_mouseY;
    
}

function lf_clearToolTipMonthsByMode(vp_mode) {
    // brisanje vseh toolTipov v trenutnem mode
    let listIndex;
    let validTips = 0;
    for (listIndex = 1; listIndex <= nrToolTips; listIndex++) {
        if (arrToolTipMode[listIndex] !== vp_mode) {
            //---- ta mesec ni za brisanje
            validTips += 1;
            if (validTips !== listIndex) {
                // treba ga je prepisati nižje dol, ker smo jih vmes že nekaj pobrisali
                arrToolTipMonth[validTips] = arrToolTipMonth[listIndex];
                arrToolTipMode[validTips] = arrToolTipMode[listIndex];
                arrToolTipPlace[validTips] = arrToolTipPlace[listIndex];
                arrToolTipTimeSlice[validTips] = arrToolTipTimeSlice[listIndex];
                arrToolTipY[validTips] = arrToolTipY[listIndex];
            }
        };
    }
    //---- v tabeli je zdaj toliko tipsov
    nrToolTips = validTips;
    //---- če ni več nobenega regularnega toolTip-a
    if (nrToolTips <= 0) {
        arrToolTipMonth.length = 0;
        arrToolTipMode.length = 0;
        arrToolTipPlace.length = 0;
        arrToolTipTimeSlice.length = 0;
        arrToolTipY.length = 0;
    }
}
